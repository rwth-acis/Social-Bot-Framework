...
def load_model(model, ckpt, session, name, initTable=True):
...
def create_or_load_model(model, model_dir, session, name, tableInit=True):
  """Create translation model and initialize or load parameters in session."""
  latest_ckpt = tf.train.latest_checkpoint(model_dir)
  if latest_ckpt:
    model = load_model(model, latest_ckpt, session, name, tableInit)
  else:
    start_time = time.time()
