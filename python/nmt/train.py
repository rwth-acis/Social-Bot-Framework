...
def run_sample_decode(infer_model, infer_sess, model_dir, hparams,
                      summary_writer, src_data, tgt_data,initTable=True):
  """Sample decode a random sentence from src_data."""
  with infer_model.graph.as_default():
    loaded_infer_model, global_step = model_helper.create_or_load_model(
        infer_model.model, model_dir, infer_sess, "infer",initTable)
...
def run_internal_eval(
    ...
    use_test_set=True,initTable=True):
  """Compute internal evaluation (perplexity) for both dev / test."""
  with eval_model.graph.as_default():
    loaded_eval_model, global_step = model_helper.create_or_load_model(
        eval_model.model, model_dir, eval_sess, "eval",initTable)
...
def run_external_eval(infer_model, infer_sess, model_dir, hparams,
  ...
  with infer_model.graph.as_default():
    loaded_infer_model, global_step = model_helper.create_or_load_model(
        infer_model.model, model_dir, infer_sess, "infer", False)
  ...
  while global_step < num_train_steps:
    ...
    if global_step - last_eval_step >= steps_per_eval:
      ...
      run_sample_decode(infer_model, infer_sess,
                        ...
                        sample_tgt_data,False)
      run_internal_eval(
          eval_model, eval_sess, model_dir, hparams, summary_writer, True, False)
