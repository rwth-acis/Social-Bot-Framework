  ...
  function Activity(entityId, sender, text, timestamp) {
    ...
    if (_sender)
      _$node = $(
        _activityBoxTemplate({
          heading: y.share.userList.get(_sender)
            ? y.share.userList.get(_sender)[CONFIG.NS.PERSON.TITLE]
            : "",
          text: _text,
          color: y.share.userList.get(_sender)
            ? Util.getColor(y.share.userList.get(_sender).globalId)
            : "#000000",
    ...
    this.trackable = function () {
      ...
      _$node.hover(
        ...
        function (event) {
          // need to reset the border to initial color
          var initialColor = y.share.userList.get(_sender)
            ? Util.getColor(y.share.userList.get(_sender).globalId)
            : "#000000";
          $(this).css("border", "3px solid " + initialColor);
        }
      );
